...
<template>
  <div class="col-xl-6 col-lg-4 col-md-6 col-sm-6 col-xs-12 mt-2">
    <div class="card mb-3 shadow border border-primary">
      <div class="row g-0">
        ...
        <div class="col-xl-8 p-3">
          <div class="card-body p-0">
            <h5 class="card-title text-primary">{{ favoriteProduct.name }}</h5>
            <h6 class="card-text my-4">
              This product is rated from
              <span class="fw-bold text-primary-emphasis"
                >{{ favoriteProduct.rating.count }}
              </span>
              buyers, and has a
              <span class="fw-bold text-warning-emphasis"
                >{{ favoriteProduct.rating.stars }} / 5</span
              >
              average stars from the people that purchased it.
            </h6>
            <h4 class="card-text">
              Price:
              <span class="fw-bold text-warning-emphasis"
            ...
            <p class="text-primary-emphasis fw-semibold">
              Times added to favorites: {{ favoriteProduct.timesAdded }}
            </p>
          </div>
          <div class="card-bottom mt-4 d-flex gap-2 justify-content-between">
            ...
            >
              <p class="text-success fw-semibold fs-5">
                Added <i class="bi bi-bag-check"></i>
              </p>
            </div>
            <button
              @click="addToCart"
              class="btn btn-primary w-75 mt-4 fw-semibold"
            >
              Add to<i class="bi bi-bag ms-2"></i>
            </button>
            <button
              @click="removeFromFavorites(favoriteProduct.id)"
              class="btn btn-outline-primary w-25 mt-4 p-1 shadow"
            >
